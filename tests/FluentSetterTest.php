...
class FluentSetterTest extends BaseTest {
    ...
    {
        /** @var Handler $handler */
        $json = [GitLabHook::EVENT_TYPE => 'push', 'ref' => 'refs/heads/master', 'rand' => rand(100, 999)];
    ...
    {
        /** @var Handler $handler */
        $json = [GitLabHook::EVENT_TYPE => 'push', 'ref' => 'refs/heads/master', GitLabHook::REPOSITORY => [GitLabHook::REPOSITORY_NAME => 'testing/testing'], 'rand' => rand(100, 999)];
        $this->client->get('/fluent-setters/on-push-for-repo.php', json_encode($json))
            ->saveResponseHandler($handler);
        $this->assertEquals($json, $handler->getFilteredResponse());

        $json = [GitLabHook::EVENT_TYPE => 'push', 'ref' => 'refs/heads/master', GitLabHook::REPOSITORY => [GitLabHook::REPOSITORY_NAME => 'testing/not-testing'], 'rand' => rand(100, 999)];
        $this->client->get('/fluent-setters/on-push-for-repo.php', json_encode($json))
            ->saveResponseHandler($handler);
    ...
    {
        /** @var Handler $handler */
        $json = [GitLabHook::EVENT_TYPE => 'push', 'ref' => 'refs/heads/master', GitLabHook::USER => [GitLabHook::USER_NAME => 'testing-user'], 'rand' => rand(100, 999)];
        $this->client->get('/fluent-setters/on-push-by-user.php', json_encode($json))
            ->saveResponseHandler($handler);
        $this->assertEquals($json, $handler->getFilteredResponse());

        $json = [GitLabHook::EVENT_TYPE => 'push', 'ref' => 'refs/heads/master', GitLabHook::USER => [GitLabHook::USER_NAME => 'not-testing-user'], 'rand' => rand(100, 999)];
        $this->client->get('/fluent-setters/on-push-by-user.php', json_encode($json))
            ->saveResponseHandler($handler);
        $this->assertEquals(['rejected' => $json], $handler->getFilteredResponse());

        $json = [GitLabHook::EVENT_TYPE => 'push', 'ref' => 'refs/heads/master', GitLabHook::USERNAME => 'testing-user', 'rand' => rand(100, 999)];
        $this->client->get('/fluent-setters/on-push-by-user.php', json_encode($json))
            ->saveResponseHandler($handler);
        $this->assertEquals($json, $handler->getFilteredResponse());

        $json = [GitLabHook::EVENT_TYPE => 'push', 'ref' => 'refs/heads/master', GitLabHook::USERNAME => 'not-testing-user', 'rand' => rand(100, 999)];
        $this->client->get('/fluent-setters/on-push-by-user.php', json_encode($json))
            ->saveResponseHandler($handler);
        $this->assertEquals(['rejected' => $json], $handler->getFilteredResponse());
    ...
    {
        /** @var Handler $handler */
        $json = [GitLabHook::EVENT_TYPE => 'push', 'ref' => 'refs/heads/master', GitLabHook::MERGE_REQUEST => [GitLabHook::SOURCE_BRANCH => 'testing'], 'rand' => rand(100, 999)];
        $this->client->get('/fluent-setters/on-push-from-branch.php', json_encode($json))
            ->saveResponseHandler($handler);
        $this->assertEquals($json, $handler->getFilteredResponse());

        $json = [GitLabHook::EVENT_TYPE => 'push', 'ref' => 'refs/heads/master', GitLabHook::MERGE_REQUEST => [GitLabHook::SOURCE_BRANCH => 'not-testing'], 'rand' => rand(100, 999)];
        $this->client->get('/fluent-setters/on-push-from-branch.php', json_encode($json))
            ->saveResponseHandler($handler);
        $this->assertEquals(['rejected' => $json], $handler->getFilteredResponse());

        $json = [GitLabHook::EVENT_TYPE => 'push', 'ref' => 'refs/heads/master', GitLabHook::MERGE_REQUEST => [], 'rand' => rand(100, 999)];
        $this->client->get('/fluent-setters/on-push-from-branch.php', json_encode($json))
            ->saveResponseHandler($handler);
        $this->assertEquals(['rejected' => $json], $handler->getFilteredResponse());

        $json = [GitLabHook::EVENT_TYPE => 'push', 'ref' => 'refs/heads/master', GitLabHook::OBJECT_ATTRIBUTES => [GitLabHook::SOURCE_BRANCH => 'testing'], 'rand' => rand(100, 999)];
        $this->client->get('/fluent-setters/on-push-from-branch.php', json_encode($json))
            ->saveResponseHandler($handler);
        $this->assertEquals($json, $handler->getFilteredResponse());

        $json = [GitLabHook::EVENT_TYPE => 'push', 'ref' => 'refs/heads/master', GitLabHook::OBJECT_ATTRIBUTES => [GitLabHook::SOURCE_BRANCH => 'not-testing'], 'rand' => rand(100, 999)];
        $this->client->get('/fluent-setters/on-push-from-branch.php', json_encode($json))
            ->saveResponseHandler($handler);
        $this->assertEquals(['rejected' => $json], $handler->getFilteredResponse());

        $json = [GitLabHook::EVENT_TYPE => 'push', 'ref' => 'refs/heads/master', GitLabHook::OBJECT_ATTRIBUTES => [], 'rand' => rand(100, 999)];
        $this->client->get('/fluent-setters/on-push-from-branch.php', json_encode($json))
            ->saveResponseHandler($handler);
        $this->assertEquals(['rejected' => $json], $handler->getFilteredResponse());
